...
static float adcAvg[3];							//get values from compass
...
float getAcclOutput(unsigned char w){
		...
		compass_acceleration(adcAvg);			// get acceleration from compass
...
float getGyroOutput(unsigned char w){
        static float tmpf;                                                      //temporary variable for complex calculations
		compass_angularVelocity(adcAvg);
        tmpf = adcAvg[w] - config.gyroOffs[w];        //remove offset
